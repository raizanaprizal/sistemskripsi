...
$barang_all = mysqli_query($conn, "SELECT * FROM jenis_barang $where ORDER BY id_jenis");
$edit_data = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['aksi'])) {
    ...
    $kategori = clean($conn, $_POST['kategori_jenis'] ?? '');
    $satuan = clean($conn, $_POST['satuan'] ?? '');
    $status = (int) ($_POST['status_aktif'] ?? 1);
    $keterangan = clean($conn, $_POST['keterangan'] ?? '');

    if ($aksi === 'tambah') {
        $exists = mysqli_fetch_row(mysqli_query($conn, "SELECT id_jenis FROM jenis_barang WHERE kode_barang='$kode' LIMIT 1"));
        if ($exists) {
            setAlert('danger', 'Kode barang sudah digunakan. Gunakan kode lain.');
        } else {
            mysqli_query($conn, "INSERT INTO jenis_barang (kode_barang,nama_barang,kategori_jenis,satuan,keterangan,status_aktif)
                VALUES ('$kode','$nama','$kategori','$satuan','$keterangan',$status)");
    ...
    if ($aksi === 'ubah') {
        ...
        mysqli_query($conn, "UPDATE jenis_barang SET nama_barang='$nama', kategori_jenis='$kategori',
            satuan='$satuan', keterangan='$keterangan', status_aktif=$status
            WHERE id_jenis=$id");
...
if (isset($_GET['hapus'])) {
    $id = (int) $_GET['hapus'];
    $cek = mysqli_fetch_row(mysqli_query($conn, "SELECT id_penilaian FROM penilaian WHERE id_jenis=$id LIMIT 1"));
    if ($cek) {
...
if (isset($_GET['edit'])) {
    $id = (int) $_GET['edit'];
    $edit_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM jenis_barang WHERE id_jenis=$id"));
}
...
<div class="card" style="padding:0;overflow:hidden">
  <div class="tbl-wrap">
    <table class="tbl">
      ...
      <tbody>
        <?php $no = 1; while ($b = mysqli_fetch_assoc($barang_all)): ?>
        <tr>
          <td><?= $no++ ?></td>
